...
export class XtabProvider implements IStatelessNextEditProvider {
	...
	private async *doGetNextEditWithSelection(
		...
		const toTextPartsHelper = (message: string): Raw.ChatCompletionContentPart[] => {
			return [{
		...
		const messages: Raw.ChatMessage[] = [
			{
				...
				content: toTextPartsHelper(this.pickSystemPrompt(promptOptions.promptingStrategy))
			},
			{
				role: Raw.ChatRole.User,
				content: toTextPartsHelper(userPrompt + `current language is ${activeDocument.languageId}`)
			}
		];
			...
			messages.push(
				{
					...
					content: toTextPartsHelper(prediction.content)
				}
			);
			messages.push(
				{
					role: Raw.ChatRole.User,
					content: toTextPartsHelper(`
对答案内容进行优化修正
